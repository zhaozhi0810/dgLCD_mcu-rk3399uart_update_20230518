...
*/
uint8_t is_cpu_update_cmd = 0;   //是rk3399的升级吗？ 0表示调试串口，1表示通讯串口
...
//	OePins_Control_Init();    //OE引脚的初始化，允许串口输出
	...
	//串口初始化
	IAP_Init(USART0);   //初始化串口0，导光面板,通讯串口
	IAP_Init(USART1);   //初始化串口1，导光面板,调试串口是串口1
	...
	SerialPutString("\r\nIAP(for dg screen mcu) 2023  start(by dazhi@jc), init ok!!\r\n");
	...
	else  //调试串口是否需要升级
	{
	  ...
	  //STM_EVAL_PBInit();
		...
		{
			if (SerialKeyPressed_Uart1((uint8_t*)&key)) //是否有调试串口的命令？？
			{
