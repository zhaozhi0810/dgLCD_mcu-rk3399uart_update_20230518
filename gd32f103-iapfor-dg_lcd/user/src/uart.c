...
/**
  ...
  * @retval None
	...
	2023-01-31
	主要是初始化串口，接收不使用中断。GPA9,GPA10（导光面板，这个是通讯串口，与3399连接）

	2023-04-07
	增加串口1，PA2，PA3，导光面板，调试串口
	但是调试信息仍然从串口1打印
...
{
	if(index == 0)  //0表示调试串口，1表示通讯串口
	{
		update_com_real = USART1;   //导光面板,调试串口是串口1
	}
	else
		update_com_real = USART0;
}
...
{
//	if(update_com_real != USART1)
		SerialPutChar_uart1(ch);
	return ch;
}
